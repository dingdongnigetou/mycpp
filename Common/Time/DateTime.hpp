...
namespace mycpp
{
	...
	public:
		...
		explicit DateTime::DateTime(const std::string& strTime) : DateTime(strTime, "%d-%d-%d %d:%d:%d") {}
		//日期字符串格式 年-月-日 时:分:秒 例:2008-02-03 9:30:20 
		explicit DateTime::DateTime(const std::string& strTime, const std::string& strFormat)
		{
	...
	public:
		...
		static DateTime Parse(const std::string& strTime) { return std::move(DateTime(strTime)); }
		static DateTime Parse(const std::string& strTime, const std::string& strFormat) { return std::move(DateTime(strTime, strFormat)); }
	...
	private:
		void DateTime::Format(const std::string& strTime, const std::string& strFormat)
		{
#if defined(UNICODE) || defined(_UNICODE)
